...
# Busca configuracoes da empresa (Logo, nome, etc)
...
# Mapeamento para filtro de itens extras
...
# Busca apenas itens da categoria correta + itens Gerais
...
selecionados = st.multiselect(f"Itens adicionais para {categoria_atual}", list(opcoes.keys()))
...
if selecionados:
    ...
    for i, sel in enumerate(selecionados):
        with cols[i % 3]:
            ...
            extras.append({"info": opcoes[sel], "qtd": q})
...
if st.button("FINALIZAR", use_container_width=True, key="f_btn"):
    ...
    # Busca texto personalizado do PDF
    with conn.cursor() as cur:
        cur.execute("SELECT texto_detalhado FROM modelos_texto WHERE usuario_id = %s AND servico_tipo = %s", (user_id, categoria_atual))
        txt_row = cur.fetchone()
        res['entrega_detalhada'] = txt_row[0] if txt_row else "Instalacao padrao realizada."

    st.success(f"Total: R$ {res['subtotal']:.2f}")
    
    # Geracao de PDF e Materiais
    st.divider()
    aba_pdf, aba_mat = st.tabs(["PDF", "Materiais"])
    with aba_pdf:
        from core.pdf.summary import render_summary_pdf
        pdf_io = io.BytesIO()
        render_summary_pdf(pdf_io, {"logo_bytes": cfg[2], "empresa": cfg[0], "whatsapp": cfg[1], "cliente": cliente, "servicos": [res], "total": res['subtotal'], "pagamento": cfg[3], "garantia": cfg[4], "validade_dias": cfg[5]})
