...
def main():
            ...
            for key, desc, val in rows:
                ...
                with c3:
                    new_val = st.number_input(
                        "Valor (R$)", value=float(val), min_value=0.0, step=1.0, key=f"pv_{key}"
                    )
    ...
    with colA:
        st.session_state.quote_meta["cliente"] = st.text_input(
            "Nome do cliente",
            value=st.session_state.quote_meta["cliente"],
            placeholder="Ex.: Maria Silva",
        )
        st.session_state.quote_meta["telefone"] = st.text_input(
            "Telefone / WhatsApp (opcional)",
            value=st.session_state.quote_meta["telefone"],
            placeholder="Ex.: 95 9xxxx-xxxx",
        )
    with colB:
        st.session_state.quote_meta["garantia"] = st.text_input(
            "Garantia",
            value=st.session_state.quote_meta["garantia"],
        )
        st.caption("Você pode adicionar mais de um serviço antes de gerar o PDF.")
    ...
    selected_label = st.selectbox("Tipo de serviço", labels)
    ...
    if not st.session_state.cart:
        ...
        st.markdown(f"**Subtotal dos serviços:** {brl(subtotal_services)}")
