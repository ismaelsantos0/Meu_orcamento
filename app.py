...
WHATSAPP = "97991728899"  # ajuste se quiser formatar
...
def calc_hastes(perimetro, espacamento, cantos=4):
    # Aproxima para garantir cobertura do perímetro.
    # Ex.: 36/2.5=14.4 -> 15 intervalos -> 16 pontos.
...
if menu == "Editar preços":
    ...
    if st.button("Gerar PDF"):
        ...
        if tipo.startswith("Cerca elétrica") and "manutenção" not in tipo:
            hastes_total, hastes_retas, hastes_canto = calc_hastes(perimetro, espacamento, cantos=int(cantos))
        ...
        # =====================================================
            ...
            # Você define o preço aqui em "Editar preços" (crie uma chave se quiser separar)
        ...
        # =====================================================
        # OUTROS SERVIÇOS (só mão de obra por enquanto)
        # =====================================================
        ...
        filename = f"orcamento_{cliente.strip().replace(' ', '_')}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
